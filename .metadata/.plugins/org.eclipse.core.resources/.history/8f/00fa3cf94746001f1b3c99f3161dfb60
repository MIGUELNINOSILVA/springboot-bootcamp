...
const op = require('@1password/sdk');
...
/**
* @typedef {{secretId: string, assumedRole: string, version: string, secretType: string, secretKey: string }} AWSSecretKey
 ...
 * @returns {String} Returns a unique secret identifier for the secret
 */
  getUniqueSecretIdentifier (secret) {
    return `${secret.secretId}-${secret.version}-${secret.secretKey}`;
  }
 ...
 */
 async _fetchSecret (secret, abortSignal) {
   ...
   if (secret.secretType === 'KeyValue' && secret.secretKey) {
    const parsedSecretData = JSON.parse(secretData.SecretString);

    return {
      value: parsedSecretData[secret.secretKey],
      id: secret.secretId,
