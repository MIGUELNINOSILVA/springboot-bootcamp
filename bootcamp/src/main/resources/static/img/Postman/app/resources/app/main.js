...
const authHandlerAdapter = require('./services/adapters/AuthHandlerAdapter');
...
const devtoolsInstaller = require('./services/devtoolsInstaller');
...
const initializePluginsHostHandler = require('./services/PluginsHostHandler').init;
const cleanupPluginsHostHandler = require('./services/PluginsHostHandler').cleanup;
...
const interceptorBridgeInstaller = require('./services/interceptorBridgeInstaller').installer;
...
const proxyCertificateService = require('./services/proxyCertificateService').proxyCertificateService;
  ...
  function attachIpcListeners () {
    ...
    if (process.platform === 'darwin') {
      ipcMain.subscribe('ui-zoom-change-event', (event, arg) => {
        ...
        // We're currently checking if we need to use the old or the new API.
        // Once we're on Electron v25 or newer, we can remove the check and use the new API.
        // Also renaming constants and variables doesn't make sense, however.
        // A "traffic light" is more descriptive than "window buttons".
        // Electron refers to it elsewhere, too, and so do some Postman's UI controls.
        if (currentWindow?.setWindowButtonPosition) {
          currentWindow.setWindowButtonPosition(newTrafficLightPosition);
        } else {
          currentWindow?.setTrafficLightPosition(newTrafficLightPosition);
        }
      });
    }
